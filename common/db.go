...
func NewInstance(filePath string) (*Dba, error) {
	_, err := os.Stat(filePath + "/tx/tx.db")
	if err != nil {
		os.MkdirAll(filePath+"/tx", 0755)
		os.Create(filePath + "/tx/tx.db")
	}
	db, err := sql.Open("sqlite3", filePath+"/tx/tx.db")
	if err != nil {
		return nil, err
	}
	return &Dba{db}, nil
...
//TODO: modify the table name and structrue
func InitDb(db *Dba) error {
	//TODO: remove useless sql code
